...
def load_and_process_data(infile, vocab_size, parser, vocab=None, log_transform=None, label_list=None):
    with codecs.open('C:\\Users\\matteo posenato\\Documents\\tesi\\codice\\MLT\\en.txt', 'r', encoding='utf-8') as input_file:
    ...
    tf_greater_100 = np.sum(list(map(lambda x: x >= 100, list(word_counts.values()))))
    if vocab is None:
        initial_vocab = {}
        vocab = copy.copy(initial_vocab)
        for w in word_counts.most_common()[tf_greater_100:]:
    ...
    for i, words in enumerate(parsed):
        ...
        if len(counter.keys()) > 0:
            ...
            values = list(counter.values())
            X[np.ones(len(counter.keys()), dtype=int) * count, list(counter.keys())] = values + X[np.ones(len(counter.keys()), dtype=int) * count, list(counter.keys())]
            y_vector = np.zeros(n_labels)
            y_vector[label_index[labels[i]]] = 1
            y.append(y_vector)
            lists_of_indices.append(indices)
    ...
    print(y.shape)
    sparse_y = sp.csr_matrix(y)
    sparse_X = sp.csr_matrix(X)
    sparse_X_indices = sp.csr_matrix(X_indices)
...
def preprocess_data(train_infile, test_infile, output_dir, vocab_size, log_transform=False):
    ...
    train_X_sparse = sp.csr_matrix(train_X)
    fh.save_sparse(train_X_sparse, os.path.join(output_dir, 'train.npz'))
