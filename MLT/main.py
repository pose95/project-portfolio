...
from train_model import train_model,evaluate
...
def main():
    ...
    parser.add_option('--train_data_file', dest='train_data_file', default="/data/lengjia/topic_model/yelp_2013_39923/yelp/yelp-train.txt.ss")
    ...
    parser.add_option('--max_utterance', dest='max_utterance', default=10)
    ...
    word2vec_file = "embedding_yelp_300.txt"
    ...
    model = JointModel( d_v=topic_vocabsize, d_e=de, d_t=dt, encoder_layers=encoder_layers, generator_layers=generator_layers,
        ...
        pretrained_embedding=pretrained_embedding, vocab = vocab_clf
    )
    ...
    optimizer_attention = torch.optim.Adam(model.parameters(), lr=lr*0.1, weight_decay=1e-8)
    train_model(log_file, model_file, model, optimizer, optimizer_attention,loss_function_clf, loss_function_attention,
                                          ...
                                          max_epochs, clf_weight,tm_weight, topic_num, temp_batch_num, maxmin,topic_file,min_epochs, cuda=cuda)
# ***********************************************       predict     ********************************************************************************
#     test_loss, test_acc, wrong_index, true_label_array, predicted_label_array, document_list, bound, cv = evaluate(
#         log_file, model_file, model, loss_function_clf, test_batch_clf, test_X_topic, test_indices, test_index_arrays, vocab_topic, topic_file, topic_num, cuda)
